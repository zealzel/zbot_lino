...
def generate_launch_description():
    ...
    def worldname_to_map(context):
        if "sim" in context.launch_configurations:
            worldname = context.launch_configurations.get(
                "worldname", default_worldname
            )
            print("worldname_to_map", worldname)
            mappath = os.path.join(
                fitrobot_install_path,
                "maps/sim",
                f"{WORLD_MAP[worldname]}",
            )
            print("mappath", mappath)
            return [SetLaunchConfiguration("map", mappath)]

    map_sim_arg = OpaqueFunction(function=worldname_to_map)
